...
import { getOrigins } from "../lib/diamonds.server";
...
export const loader = async ({ request }) => {
  ...
  return {
    ...
    origins: await getOrigins(session.shop),
...
export const action = async ({ request }) => {
  ...
  if (intent === "add") {
    ...
    const shopOrigins = await getOrigins(session.shop);
    const origin = normOrigin(fd.get("origin"), shopOrigins);
    ...
    if (!shape || !origin || !carat || !colour || !clarity || pence == null || pence < 0) {
      return { result: { ok: false, message: `Fill every field with a valid value (origin one of: ${shopOrigins.map((o) => o.key).join(", ")}; price in £).` } };
    }
  ...
  // Resolve against the shop's own origins, so a merchant who added e.g.
  // "Moissanite" can import rows for it instead of having them all rejected.
  const shopOrigins = await getOrigins(session.shop);
  const originKeys = shopOrigins.map((o) => o.key).join(", ");
  ...
  records.forEach((rec, i) => {
    ...
    const origin = normOrigin(rec.origin, shopOrigins), carat = normCarat(rec.carat);
    ...
    if (!origin) e.push(`origin '${rec.origin}' is not one of: ${originKeys}`);
...
export default function PriceData() {
  const { rows, total, page, shapeFilter, suggest, origins } = useLoaderData();
            ...
            <adder.Form method="post" className="ds-modal-body">
              ...
              <div className="ds-form-grid">
                ...
                <label><span className="muted">Origin</span>
                  <select name="origin" className="ds-field" defaultValue={editRow?.origin || origins[0]?.key}>
